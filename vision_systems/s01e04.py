...
# https://docs.google.com/document/d/1UtLMMk7Ew8ob5CKVnArQJ-mR95yhqi7VcmHOovm5jTU/edit?usp=sharing


# global variable that will be asynchronously changed in the callback
img_todo_1 = None
...
def mouse_callback_todo_1(event, x, y, flags, userdata):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        img_todo_1 = cv2.rectangle(img_todo_1, (x-50, y-50), (x+50, y+50), (255, 0, 0), thickness=5, lineType=cv2.LINE_8)
    elif event == cv2.EVENT_RBUTTONDOWN:
        img_todo_1 = cv2.circle(img_todo_1, (x, y), 50, (0, 0, 255), thickness=5, lineType=cv2.LINE_4)
    elif event == cv2.EVENT_MBUTTONDOWN:
        img_todo_1 = cv2.circle(img_todo_1, (x, y), 50, (0, 0, 255), thickness=5, lineType=cv2.LINE_AA)
...
def todo_1():
    ...
    cv2.namedWindow('img_todo_1')
    cv2.setMouseCallback('img_todo_1', mouse_callback_todo_1)

    key = ord('a')
    while key != ord('q'):
        # additional option to better visualize the different circle drawing methods
        if key == ord('w'):
            img_todo_1 = cv2.resize(img_todo_1, dsize=None, fx=2, fy=2, interpolation=cv2.INTER_NEAREST)

        cv2.imshow('img_todo_1', img_todo_1)
        key = cv2.waitKey(50)
...
def todo_2():
    ...
    cv2.namedWindow('img_todo_2')
    cv2.setMouseCallback('img_todo_2', mouse_callback_todo_2)
    ...
    while key != ord('q'):
        if len(pts_1_todo_2) >= 4:
            ...
            perspective_transform = cv2.getPerspectiveTransform(pts_1_np, pts_2)
            dst = cv2.warpPerspective(img_todo_2, perspective_transform, (300, 300))
            cv2.imshow('dst', dst)
            img_todo_2 = img_todo_2_original.copy()
            pts_1_todo_2.clear()

        cv2.imshow('img_todo_2', img_todo_2)
        key = cv2.waitKey(50)
...
def homework_3():
    ...
    while key != ord('q'):
        if len(pts_2_todo_3) >= 4:
            ...
            perspective_transform = cv2.getPerspectiveTransform(pts_1, pts_2_todo_3_np)
            dst = cv2.warpPerspective(
                img_pug, perspective_transform, (img_gallery_todo_3.shape[1], img_gallery_todo_3.shape[0])
            )
            dst_mask = cv2.warpPerspective(
                img_mask, perspective_transform, (img_gallery_todo_3.shape[1], img_gallery_todo_3.shape[0])
            )
